...
class Main():
        ...
        while self.is_running:
            ...
            match opcao:
                ...
                case 1:
                    ...
                    continue
                case 2:
                    self.criar_conta(self.clientes, self.contas)
    ...
    def escolher_conta_cliente(
        self, cliente: PessoaFisica
    ) -> Union[ContaCorrente, None]:
        ...
        while True:
            try:
                opcao = (
                    self.ler_int("\n- - - - - - -> Informe a conta que deseja usar: ")
                    - 1
                )
    ...
    def existe_cliente(
        self, cpf: str, clientes: List[PessoaFisica]
    ) -> Union[PessoaFisica, None]:
    ...
    @log_transacao
    def criar_cliente(self, clientes: list) -> None:

        cpf = self.ler_e_verificar_cpf()
        if not cpf:  # O cpf é inválido?
            return

        if self.existe_cliente(
            cpf, clientes
        ):  # Já existe um cliente com este mesmo cpf?
    ...
    @log_transacao
    def criar_conta(self, clientes: list, contas: list) -> None:

        cpf = self.ler_e_verificar_cpf()
        if not cpf:  # O cpf é inválido?
            return

        cliente = self.existe_cliente(cpf, clientes)
    ...
    @log_transacao
    def depositar(self, clientes: list) -> None:
    ...
    @log_transacao
    def sacar(self, clientes: list) -> None:
    ...
    @log_transacao
    def exibir_extrato(self, clientes: list) -> None:
        ...
        tipo_transacao = self.ler_string(
            "\n - - -> Insira o tipo de transação (Saque/ Deposito/ Todos): "
        ).lower()
        ...
        for transacao in conta.historico.gerar_relatorio(tipo_transacao):
            ...
            data_transacao = datetime.astimezone(
                transacao["data"], pytz.timezone("America/Sao_Paulo")
            )
            transacao_formatada = datetime.strftime(
                data_transacao, ("%Y/%m/%d %H:%M:%S")
            )

            extrato += f"|   {transacao["tipo"]}" + "|".rjust(
                11 - len(str(transacao["tipo"]))
            )
            extrato += f"   {transacao["valor"]}" + "|".rjust(
                23 - len(str(transacao["valor"]))
            )
            extrato += f"   {transacao_formatada}" + "|".rjust(
                21 - len(transacao_formatada)
            )
            extrato += "\n"
